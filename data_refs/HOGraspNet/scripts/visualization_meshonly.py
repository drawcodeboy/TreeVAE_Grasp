...
if __name__ == '__main__':
    ...
    HOG = HOGMeshDataset(setup, split, db_path=db_path)
    ...
    for idx, sample in enumerate(HOG_loader):
        ...
        label_path = sample['label_path'][0]
        seq_name = label_path.split('/')[-4]
        trial_name = label_path.split('/')[-3]
        img_name = os.path.basename(label_path).replace('.json', '.png')
